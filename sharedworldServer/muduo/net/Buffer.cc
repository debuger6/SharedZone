...
ssize_t Buffer::readFd(int fd, int* savedErrno)
{
  ...
  char extrabuf[65536]; //分配64k栈上的缓冲区，足够大，避免调用ioctl获取可读数据的大小；当buffer_空间不足时，将多余的数据存入到extrabuf，再追加到buffer_中，extrabuffer是栈上的空间，函数执行完后会自动释放，这样避免了程序一开始就分配很大的动态空间给buffer_造成空间浪费。
  struct iovec vec[2];
